...
**生图 API（可选）**：想让角色**真的发图**（今天的晚饭、路上的猫、画了一半的稿子），在「设置 → 生图 API」里填一套 OpenAI 兼容的文生图接口：

| 字段 | 说明 |
|------|------|
| **URL** | 填到 `/v1` 即可，末尾的 `/images/generations` 会自动补 |
| **Key** | 生图服务商的密钥 |
| **生图模型** | 如 `dall-e-3`、`Kwai-Kolors/Kolors`、`flux-schnell`——跟聊天模型不是一个 |
| **图片尺寸** | 点预设（512×512 / 1024×1024 / 1024×1792…）或手填 `宽x高`，留空则用服务端默认 |
| **生图提示词** | 画风模板，统一所有图的风格。写 `{prompt}` 的地方会被角色的描述替换；不写就当前缀 |

开启并保存后，角色的提示词里会多出 `[[SEND_IMAGE: 画面描述]]` 这条能力，它自己决定什么时候发图；出图走的是**你自己的生图额度**，关掉即停。高级选项里还有负向提示词、返回格式（默认 `b64_json`，图片跟着聊天记录一起存、一起备份）、附加参数 JSON（塞服务商私有字段如 `steps` / `cfg_scale`）。填完点「🎨 测试生图」会真出一张图给你看效果。
